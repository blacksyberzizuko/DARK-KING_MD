    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                if (fileSize >= 1800) throw new Error(' 👻 size big..\n\n')
                let opt = {}
                    ...
                    templateButtons.push(...(
                        url.map((v, i) => [v, urlText[i]])
                            .map(([url, urlText], i) => ({
                                ...
                                urlButton: {
                                    displayText: !nullish(urlText) && urlText || !nullish(url) && url || 'dark-hyper',
                                    url: !nullish(url) && url || !nullish(urlText) && urlText || 'https://wa.me/message/V4RHGWZVB40F01'
                                }
                            })) || []
                    ))
                }
                    ...
                    templateButtons.push(...(
                        call.map((v, i) => [v, callText[i]])
                            .map(([call, callText], i) => ({
                                index: templateButtons.length + i + 1,
                                callButton: {
                                    displayText: !nullish(callText) && callText || !nullish(call) && call || 'dark-hyper',
                                    phoneNumber: !nullish(call) && call || !nullish(callText) && callText || '+94770378874'
                                }
                            })) || []
                    ))
                }
